...
${body.split('\n').map(line => '  ' + line).join('\n')}
    ...
    generateInterface(name, properties) {
    ...
    generateReactComponent(name, props, hooks = []) {
        let code = '';
        // Imports
        code += `import React`;
        if (hooks.length > 0) {
    ...
    generateAPI(endpoint, method, params) {
    ...
    generateTest(functionName, testCases, framework = 'vitest') {
        let code = '';
        // Imports
        if (framework === 'vitest') {
